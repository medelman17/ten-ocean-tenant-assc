...
// Add interfaces for database types
interface UserData {
  email: string
  user_profiles?: {
...
export async function fetchFloorCaptainAssignments(): Promise<FloorCaptainAssignment[]> {
  ...
  return data.map((assignment) => {
    ...
    let user = undefined
    if (assignment.users) {
      // Use type assertion with the correct type
      const users = assignment.users as unknown as UserData
      user = {
        email: users.email,
        firstName: users.user_profiles?.first_name || null,
        lastName: users.user_profiles?.last_name || null,
        profilePictureUrl: users.user_profiles?.profile_picture_url || null,
      }
    }

    // Create a valid FloorCaptainAssignment object
    return {
      id: camelCaseAssignment.id as string,
      userId: camelCaseAssignment.userId as string,
      floorNumber: camelCaseAssignment.floorNumber as number,
      assignedAt: camelCaseAssignment.assignedAt as string,
      assignedBy: camelCaseAssignment.assignedBy as string | null,
      user,
    } as FloorCaptainAssignment
  })
}
...
export async function fetchAvailableFloors(): Promise<number[]> {
  ...
  const { data, error } = await supabase.from("units").select("floor").order("floor")
...
> {
  ...
  return data.map((user) => ({
    id: user.id,
    email: (user.users as unknown as UserData)?.email || "",
 ...
 */
export async function assignFloorCaptain(userId: string, floorNumber: number): Promise<{ success: boolean; error?: string; id?: string }> {
  // Ensure the user is an admin
  const { user: adminUser } = await checkServerAuth([ROLES.ADMIN])
  ...
  const { data: existingAssignments } = await supabase.from("floor_captain_assignments").select("id").eq("user_id", userId).eq("floor_number", floorNumber)
  ...
  const { data: existingRoles } = await supabase
    ...
    .select("id")
    .eq("user_id", userId)
    .eq("role_id", (subquery: RoleSubquery) => {
      return subquery.from("roles").select("id").eq("name", "FloorCaptain").single()
    })

  if (!existingRoles || existingRoles.length === 0) {
    // Get the floor captain role ID
    const { data: floorCaptainRole } = await supabase.from("roles").select("id").eq("name", "FloorCaptain").single()
 ...
 */
export async function removeFloorCaptainAssignment(assignmentId: string): Promise<{ success: boolean; error?: string }> {
  // Ensure the user is an admin
  await checkServerAuth([ROLES.ADMIN])
  ...
  const { error } = await supabase.from("floor_captain_assignments").delete().eq("id", assignmentId)
