...
export async function fetchAssignedFloors(): Promise<FloorAssignment[]> {
  const { user } = await checkServerAuth([ROLES.FLOOR_CAPTAIN, ROLES.ADMIN])

  const supabase = await createClient()

  // Get user roles to check if they're an admin
  const { data: userRoles } = await supabase.from("user_roles").select("roles:role_id(name)").eq("user_id", user.id)

  const isAdmin = userRoles?.some((ur) => (ur.roles as unknown as RoleData)?.name === ROLES.ADMIN)

  if (isAdmin) {
    const { data: floorNumbers } = await supabase.from("units").select("floor").order("floor")
  ...
  const { data: assignments, error } = await supabase.from("floor_captain_assignments").select("*").eq("user_id", user.id).order("floor_number")
...
export async function fetchFloorInfo(floorNumber: number): Promise<FloorInfo | null> {
  await checkServerAuth([ROLES.FLOOR_CAPTAIN, ROLES.ADMIN])

  const supabase = await createClient()

  // Get all units on this floor
  const { data: units, error: unitsError } = await supabase.from("units").select("id").eq("floor", floorNumber)
...
export async function fetchFloorResidents(floorNumber: number): Promise<ResidentProfile[]> {
  ...
  return formattedResidents as unknown as ResidentProfile[]
}
...
export async function fetchFloorAnnouncements(floorNumber: number): Promise<FloorAnnouncement[]> {
  ...
  return announcements.map((announcement) => ({
    ...
    createdBy: {
      ...
      displayName: (announcement.user_profiles as unknown as { display_name?: string })?.display_name,
      firstName: (announcement.user_profiles as unknown as { first_name?: string })?.first_name,
      lastName: (announcement.user_profiles as unknown as { last_name?: string })?.last_name,
    },
 ...
 */
export async function createFloorAnnouncement(formData: FormData | Record<string, string>): Promise<{ success: boolean; error?: string }> {
  const { user } = await checkServerAuth([ROLES.FLOOR_CAPTAIN, ROLES.ADMIN])
 ...
 */
export async function contactFloorResidents(floorNumber: number, subject: string, message: string): Promise<{ success: boolean; error?: string }> {
  await checkServerAuth([ROLES.FLOOR_CAPTAIN, ROLES.ADMIN])
